...
namespace TestCache
{
    public class BackgroundTransService10 : IHostedService, IDisposable
    {
        private Timer _timer;
        private int executionCount = 0;
        private readonly ILogger<BackgroundTransService10> _logger;
        private IIDTPTransCache _idtpTransCache;
        private string lastStoredValue = String.Empty;

        public BackgroundTransService10(ILogger<BackgroundTransService10> logger, IIDTPTransCache idtpTransCache)
        {
            //_logger = logger;
        ...
        private void DoWork(object state)
        {
            ...
            connection.Open();
            if (count == 1)
            { //do the following iteration only once since it'll 
